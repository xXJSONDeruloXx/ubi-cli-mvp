...
- `ubi setup` now provides the one-time operator entry point: it reuses or creates CLI auth, resolves/creates one owner-only shared prefix, asks before installing the pinned verified official client (`--yes` for unattended consent), saves the default profile, and opens Connect only when remembered state is absent. `setup --check` inspects the canonical prefix without starting Wine/Connect; live output was `locally-ready` with remembered-auth evidence present and zero Wine processes. A separate disposable XDG run created a fresh mode-0700 prefix, verified/installed the pinned local client, recorded distinct `pinned-installer` provenance, wrote a mode-0600 profile, emitted clean JSON, avoided client launch, and correctly reported `needs-connect-auth`. Offline opaque-file evidence still cannot prove server-side token validity.

### 2026-07-13 fresh Connect authentication trace

- A fresh pinned-client prefix was observed before, during, and after official password/MFA entry using metadata, hashes, process names, remote endpoints, and DNS names only. No keystrokes, credential values, memory, TLS payloads, or raw headers were captured.
- `user.dat` appeared first, followed by `ConnectSecureStorage.dat`, then ownership cache. Final sizes were 518 bytes, 8,124 bytes, and one 3,384-byte ownership file respectively.
- `settings.yaml` retained no username, password, or exchange code. The binary secure-storage container exposed a `RememberMeTicket` structural label but no password label; no credential value was decoded or retained.
- Relaunching the stopped prefix went directly to the authenticated library. Secure-storage content changed at the same 8,124-byte size while final user/ownership content did not, directly confirming mutable refresh/rotation behavior.
- DNS-only monitoring showed public UbiServices/websocket gateways plus UPC channel, Demux, Connect CDN, and store-session service families. No payload inspection was performed.
- Neither the separate CLI ticket nor CLI remember-me ticket appeared verbatim in Connect secure storage in plain or UTF-16 representation. Together with the earlier `MachineGuid` isolation result, this supports desktop-specific opaque remembered state, not direct CLI-session reuse.
- No supported device-code flow, SSO URI, command-line ticket handoff, or ticket-to-secure-storage conversion was identified. Pure CLI API authentication remains implemented, but first desktop authentication remains official-client UI work.
- The disposable authenticated prefix and raw observations were deleted after the sanitized findings were recorded. The canonical prefix stayed intact and no Wine/Connect process remained.
