...
### 2026-07-14 Optima neutral compatibility ports

- [x] Resolve missing inline manifest hashes through the existing ownership-service fallback.
- [x] Support bucketed `slices_v3` and legacy flat `slices` CDN paths.
- [x] Support raw-deflate legacy slices without weakening size/hash validation.
- [x] Add bounded rolling per-file slice prefetch with a separately bounded CLI control.
- [x] Add an opt-in UMU/Proton compatibility launch path with legacy CPU/NvAPI tuning.
- [x] Add a separate opt-in launcherless backend with live ownership checks, reversible hash-recorded loaders, temporary mode-0600 profiles, dedicated-prefix registry setup, and no real-password or certificate handling.
- [x] Preserve the official Connect backend unchanged and require explicit local-ticket opt-in when a legacy entitlement has no Uplay ticket ID.
...
- [x] Trace a fresh official-client authentication using metadata/DNS only; confirm `RememberMeTicket`-bearing mutable secure storage, empty password settings, automatic remembered login, and separation from the CLI session
- [ ] Continue looking for a supported desktop device-code/SSO handoff through documentation or static analysis; do not manufacture device-bound secure storage
...
- 2026-07-13: Traced first authentication in another disposable fresh prefix without capturing credential values or traffic payloads. `user.dat`, then `ConnectSecureStorage.dat`, then ownership cache appeared; settings retained no username/password. A later launch auto-authenticated and changed secure-storage content at identical size, confirming mutable remembered state. CLI ticket values were not present verbatim, no supported SSO/ticket handoff was found, and the raw prefix/trace were deleted.
- 2026-07-13: Compared Optima's launcherless Rust client at `026f0ae`. Its missing-manifest lookup, flat-slice support, raw deflate, byte progress, and Proton knowledge are useful, but its replacement-loader/password/certificate model stays outside the official bridge. Neutral downloader improvements should be ported only after containment, integrity, provenance, and test hardening.
- 2026-07-14: Added the isolated `launcherless` backend and live-validated owned product 109 through startup logos, the Splinter Cell main menu, and working input with no Connect process. The entitlement exposes no Uplay ID, so local ticket assertion requires an extra flag after live ownership verification. An initial post-logo crash identified Optima's EAX stub as missing the native DLL's ordinal 6; preserving game-supplied EAX fixed startup. Temporary profiles cleaned up and loader restore hashes passed. Xalia-disabled testing caused missing input plus recurring desktop not-responding dialogs, so Xalia remains enabled; its independent corner black square disappears with the Xalia/game process and is left as a title-specific Proton presentation quirk.
- 2026-07-14: Reconstructed all 974 files / 4,166,301,851 bytes of owned product 1847 (Assassin's Creed Chronicles India), then SHA-256-verified the complete tree in 4.1s with zero network requests. This legacy manifest exposed zero-length modern slice tokens, legacy-only per-file slice lists, zero-byte files, and already-uncompressed data beginning with zlib-like bytes; normalization and regression tests now cover each case. A launcherless R1 run obtained the real Ubisoft-issued game ticket, started the game executable without any Connect process, then restored the original loader hash and removed all ticket profiles/backups on exit. The full format/lint/typecheck/109-test gate passed.
- 2026-07-14: Reconstructed all 679 files / 3,616,801,176 bytes of owned product 1651 (Assassin's Creed Chronicles China) from 2,239 unique slices with zero URL refreshes, then verified the full tree in 4.7s with zero network. Its real-ticket R1 path created the DX11 game window without Connect and restored the exact original loader hash after clean exit. The UE3 window initially appears minimized under GNOME/GE-Proton, so repeatable literal `--game-arg` support was added for title runtime flags; menu/input remain conservatively unclaimed pending direct visual confirmation.
