...
| `ubi download-urls 3539`          | validated live                          | Returned a live ownership-token expiration and signed download-service URLs for `.manifest`, `.metadata`, and `.licenses` assets for owned Demux product `3539`.[4][5][19]                                     | `node dist/index.js download-urls 3539`                       | This still stops at asset URL retrieval; it does not yet fetch and reconstruct content slices.                                                                                                                                                         |
...
| Manifest parser tests             | validated with tests                    | `tests/manifest-parser.test.ts`, `tests/manifest-summary.test.ts`, and `tests/manifest-service-live.test.ts` assert fixture parsing, size summarization, and live-manifest plumbing.[3][17][18][19]            | `npm test -- --run tests/manifest-parser.test.ts`             | Stable regression coverage for public and live manifest processing helpers.                                                                                                                                                                            |
...
- Current automated test count: 25 tests across 15 test files.
