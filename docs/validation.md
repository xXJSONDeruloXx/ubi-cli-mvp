...
| `ubi download-game 109`                                                 | validated live, still experimental      | Deterministic hash-derived CDN paths reduced 5,844 slice lookups from 187,008 candidate paths to 5,844. A resumed full run reconstructed all 5,320 files / 2,545,474,351 bytes in 5m49s; a second SHA-256 resume scan verified all outputs in 14s with zero network transfer.[19]                                                          | `node dist/index.js download-game 109 --all --yes --workers 4 --output-dir /games/splinter-cell`                      | This is the strongest downloader-style validation in the repo so far, but it remains experimental rather than a launcher-grade install/update path.                                                                                                    |
| `ubi run /games/splinter-cell`                                          | validated live, client boundary found   | A dry run selected `system/SplinterCell.exe`. Direct Wine reached the expected missing-Uplay-client error; after installing the official Ubisoft Connect client in an isolated prefix, Connect mapped its UI, accepted the game's Uplay API connection, and began a game session. The unauthenticated client then closed the game process. | Run `--dry-run` first, then use an explicit executable and a user-managed prefix/client.                              | `run` starts in the executable directory but does not automate Connect authentication, entitlement, DRM, or prefix setup. Interactive client authentication remains a user boundary.                                                                   |
...
- the completion transferred 2,072,729,411 new bytes with 5,706 network fetches, 36 in-process reuse hits, 134 verified existing files, and zero URL refreshes
- a second full run SHA-256-verified all 5,320 state entries in 14s with `sliceReferenceCount: 0`, `uniqueSliceCount: 0`, `bytesDownloaded: 0`, and `skippedExistingFiles: 5320`
...
- `run` dry-run selected `system/SplinterCell.exe`; direct Wine loaded the bundled Uplay DLLs and reached a missing-client error rather than entering the engine
- the current official Ubisoft Connect client installed and mapped its UI in an isolated Wine prefix; retrying the game caused Connect to accept the Uplay API process, record product id 0, and begin a game session before the unauthenticated fresh client closed the process
- no session, credential, registry registration, or DRM workaround was transferred or fabricated; interactive desktop-client authentication and entitlement handling remain outside the CLI
...
7. The CLI is still not a launcher-grade installer, updater, repair tool, or game runner. `run` now uses the executable directory as its working directory, but Wine-prefix setup plus legitimate Ubisoft Connect authentication, entitlement, and DRM handling remain external user responsibilities.
...
It does **not** yet provide install/update/repair orchestration or Ubisoft Connect integration. The thin `run` command can start a selected executable, but client-backed builds still require a legitimately installed and interactively authenticated desktop client.
