...
| `ubi download-game 109`                                                 | partially validated live                | A first-pass whole-tree Splinter Cell reconstruction run progressed far enough to expose two important live behaviors: older slices can be zlib-framed instead of zstd, and a long-running full-game pass can later hit signed slice URL `403` failures.[19] | `node dist/index.js download-game 109 --output-dir /tmp/splinter-cell-download`                                       | Useful for research and partial reconstruction, but not yet reliable enough to claim successful full-game downloading.                                                                                                                                 |
...
- Current automated test count: 42 tests across 17 test files.
...
- `download-game 109 --output-dir /tmp/splinter-cell-download` was useful as a whole-tree Splinter Cell probe: it exposed zlib-framed slice payloads on an older title and later hit signed slice URL `403` failures, confirming that full-game downloader reliability still needs refresh/resume orchestration
