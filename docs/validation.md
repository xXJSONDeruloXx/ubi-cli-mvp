...
| `ubi download-game 109`                                                 | validated live                          | A clean cache/output run reconstructed all 5,320 files / 2,545,474,351 bytes in 2m55s with 5,844 network fetches, zero cache hits, and zero URL refreshes. A second full pass SHA-256-verified every output in 10s with zero network transfer.[19]                                                                                                                 | `node dist/index.js download-game 109 --all --yes --workers 4 --output-dir /games/splinter-cell`                           | Deterministic hash-derived paths reduced 187,008 candidate lookups to 5,844; whole-tree behavior remains title/build dependent.                                                                                                                        |
| `ubi connect-seed /games/splinter-cell`                                 | validated live                          | After Connect created and paused its official product-109 download, a dry run found 3,083 matching staged files and 2,237 mismatches. Seeding atomically copied 2,519,583,113 bytes in 22s; a second comparison matched 5,320/5,320. Connect then finalized instantly, produced its own state/manifest, and all 5,320 final payload hashes matched the CLI source. | Run `--dry-run`, then `--yes`, only after pausing and fully exiting the official client.                                   | The command reads Connect's product registration but never modifies registry/state/manifest metadata. Connect performs authoritative verification and entitlement handling.                                                                            |
| `ubi run /games/splinter-cell --connect-product-id 109`                 | validated live                          | The pinned official client installed and authenticated in an explicit Wine prefix. After official finalization, the registered `uplay://launch/109/0` protocol started Connect and `SplinterCell.exe` from the official install directory without clicking Play; the game ran successfully and was exited normally.                                                | Use `--connect --connect-ready --connect-product-id 109` after Connect shows Play.                                         | Initial Connect credential/MFA entry remains manual and inside the official UI. The CLI never transfers its web session, submits credentials, or fabricates client registration.                                                                       |
| `ubi play 109`                                                          | validated live                          | A mode-0600 profile stored only product/source/shared-prefix paths. `play 109` launched the registered URI without client clicks, detected the game lifecycle, and automatically stopped Connect after normal game exit; zero Wine/Connect processes remained.                                                                                                     | Configure `connect-profile default` and `connect-profile set 109 --install-dir ... --executable ...`, then run `play 109`. | This removes repeated launch and post-game promotional-modal interaction while keeping entitlement/DRM inside Connect.                                                                                                                                 |
| `ubi connect-prefix clone`                                              | validated live with security caveat     | A stopped 3.74-GB authenticated prefix cloned on Btrfs in 637ms using reflinks and initially opened signed in. Launching the clone rotated/invalidated the source session, proving clones are one-way migrations rather than parallel authenticated templates; the clone was immediately deleted.                                                                  | Use only with `--include-auth --yes`; the default refuses a non-reflink full copy.                                         | Prefer one owner-only shared prefix. A clone contains sensitive remembered auth/DPAPI state and only one lineage should remain active.                                                                                                                 |
...
- Current automated test count: 64 tests across 20 test files.
...
- an initial whole-tree Splinter Cell probe exposed zlib-framed slices, URL-expiration/resume needs, and severe 32-prefix CDN probing overhead; the pre-optimization baseline reached only 138 files / 16.5 MB in 31m32s
- deriving each slice's one deterministic CDN path reduced the 5,844-slice manifest from 187,008 requested candidates to 5,844 paths; all URLs resolved in 31.5s, and a resumed `download-game 109 --all --yes --workers 4` run completed all 5,320 files / 2,545,474,351 bytes in 5m49s
- the first optimized completion transferred 2,072,729,411 new bytes with 5,706 network fetches, 36 in-process reuse hits, 134 verified existing files, and zero URL refreshes; its verified resume took 14s
- a later reset removed the game, Wine prefix, cache, and product-specific resume state while preserving only repo changes and CLI authentication; a genuinely clean run transferred 2,082,765,818 bytes through all 5,844 network slices and completed 5,320 files / 2,545,474,351 bytes in 175s
- a second pass over that clean tree SHA-256-verified all 5,320 outputs in 10s with zero slices and zero downloaded bytes
- `slice-urls 109 --limit 5844` also completed with empty stderr after switching the client to reuse one `download_service` connection, confirming the repeated listener warning was caused by repeated connection creation
- direct Wine loaded the bundled Uplay DLLs and reached a missing-client error; guided support now requires an explicit prefix and executes only a SHA-256-pinned official installer with a PE certificate table
- after first client authentication the owned library showed Splinter Cell as uninstalled. Launching the raw executable opened Connect but did not register the external reconstructed tree; following the Download prompts therefore attempted a duplicate official transfer
- the first clean download attempt never created staging because Connect failed five ownership-signature requests. Retrying with the Wine overrides used in the earlier successful prefix obtained authorization and logged official download initiation for product 109/build `D93F248F70E28B2F7675CCB9A34E1FE472755B59`
- after that transfer began, it was paused and Connect was fully exited. `connect-seed --dry-run` found 3,083 matching files and 2,237 mismatches totaling 2,519,583,113 bytes; the guarded write completed in 22s using contained atomic publication
- a post-seed dry run matched all 5,320 files with zero bytes remaining. Connect Resume briefly displayed an invalid time estimate, then immediately finalized and changed the UI to Play
- Connect moved the staging payload into its registered install directory, wrote its own `uplay_install.manifest`/state, and removed the product staging directory. All 5,320 finalized payload files matched the CLI source by SHA-256
- clicking Play launched `SplinterCell.exe` successfully. A separate test of the registered `uplay://launch/109/0` handler also started Connect and the game without a Play-button interaction
- one shared owner-only prefix retained the official client's remembered authentication. A non-secret mode-0600 profile reduced subsequent launch to `ubi play 109`; the command detected game startup/exit and stopped Connect automatically, leaving no promotional modal or Wine process behind
- a same-machine Btrfs reflink clone of the stopped prefix completed in 637ms and opened authenticated, but its token refresh invalidated the source prefix. This makes whole-prefix auth copying a one-way migration only; parallel cloned prefixes are not supported or recommended
- no web session, desktop credential, registry registration, client database entry, state/manifest metadata, DLL, or DRM mechanism was transferred or fabricated; first authentication, initial official download-state creation, and final client verification remained official-client operations
...
7. Guided Connect integration is validated for one legacy title/build, not a general launcher-grade installer. The CLI can bootstrap a prefix, seed a paused official download, and invoke a registered product URI, but first client authentication/MFA, official download-state creation, entitlement, and final verification still belong to Ubisoft Connect. Other titles may use different staging or registration behavior.
...
- a guarded, hash-verified bridge from one paused official Connect download to a reconstructed payload, followed by official verification and URI-based launch

It does **not** yet provide universal install/update/repair orchestration or unattended first-time Ubisoft Connect authentication. The validated product-109 path still needs one official client login and one official download-state initialization before the CLI can seed and subsequently launch it without manual Play interaction.
